...
plot_m = get_plot(u'M1、M2同比增速', pct=True)
...
plot_loan = get_plot(u'月度新增人民币贷款', pct=True)
plot_loan.line('date', 'loan', source=source_loan, line_width=2, legend=u'月度新增人民币贷款')

plot_deposit = get_plot(u'活期、定期存款同比增速', pct=True)
...
plot_rate = get_plot(u'人民币贷款加权平均利率', pct=True)
plot_rate.line('date', 'r', source=source_rate, line_width=2, legend=u'人民币贷款加权平均利率')

plot_base = get_plot(u'1年期贷款、存款基准利率', pct=True)
...
plot_bad = get_plot(u'商业银行不良贷款余额、比例与拨备覆盖率', pct=True)
...
plot_profit = get_plot(u'银行利润占产业利润的比例', pct=True)
...
curdoc().add_root(column(plot_m, plot_loan, plot_deposit, plot_rate, plot_base, plot_bad, plot_conf, plot_cny, plot_profit))
